...
$.fn.jqPuzzle = function(settings, texts) {
	...
	// default settings
	var defaults = {		
		rows: puzzle_size, 					// number of rows [3 ... 9]
		cols: puzzle_size,		 			// number of columns [3 ... 9]
		hole: puzzle_size*puzzle_size,					// initial hole position [1 ... rows*columns]
		shuffle: false,				// initially show shuffled pieces [true|false]
		numbers: true,				// initially show numbers on pieces [true|false]
		language: 'pl',				// language for gui elements [language code]
	...
	var i18n = {
		...
		pl: {
			shuffleLabel: 			'Wymieszaj',
			toggleOriginalLabel: 	'Oryginał',
			toggleNumbersLabel: 	'Numery',
			confirmShuffleMessage: 	'Czy napewno chcesz wymieszać puzzle?',
			movesLabel:				'ruchy',
			secondsLabel:			's'
		},
		fr: {
	...
	return this.filter('img').each(function(){
		...
		$srcImg.one('load', function() {
			...
			var getOffset = function(row, col) {
           	...
           	var updatePuzzlesOrder = function(){
				for(var i = 0; i < $pieces.length+1; i++) {
			        kawalki[i] = hole+1;
				}
				for(var i = 0; i < $pieces.length; i++) {
				   var j=parseInt($pieces.eq(i).attr('current'));
	      		   kawalki[j] = i+1;
	      		}
//         		$('#ramka').text(kawalki.toString());
        	}
			// shuffle pieces
			...
			$sendPuzzle = $protoButton.clone()
				.text("Wyslij")
				.appendTo($controls);
			
			$sendPuzzle.css('float','right');
            ...
            $sendPuzzle.click(function() {
//                var selected_algorithm = ($("input[name='selected_algorithm']:checked").val());
//                                                                , algorithm: selected_algorithm
                $.getJSON('query', { x: kawalki, size: puzzle_size}, function(data) {

                    document.getElementById('iframka').src = data['result_fname'].toString();

                    $('#ramka').text('');
                    $('#ramka').append(' ');
                    $('#ramka').append('<a href=\"'+data['result_fname'].toString()+'\"> Wynik</a> &nbsp &nbsp &nbsp dlugosc=');
                    $('#ramka').append( data['result_len'].toString() );
